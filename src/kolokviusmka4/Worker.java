            ...
            if(line.equals("hello")) {
                ...
                while((line = socketReader.readLine()) != null){
                    String[] parts = line.split(" ");
                        lock.lock();
                        String prom = fileMaxReader.readLine(); //mora prvo da se prochita
                        max = Integer.parseInt(prom);
                        if(parts.length > max){  //max ne e lokalna promenliva, toa e brojche sho se chita of FILE!!!
                            max = parts.length;
                            fileMaxWriter.write(max);
                            lock.unlock();
                    }
                        if(line.equals("MAX")) { //logikata treba da se pishi nadvor of IF-ot bidejkji site lini se sporedvat
                            lock.lock();
